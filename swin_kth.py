...
# 🟢 [ADD THIS LINE] Set the Hugging Face endpoint to a stable mirror
...
IMG_SIZE = 224  # 使用 Swin 预训练的原生最佳尺寸
BATCH_SIZE = 32  # Swin-Tiny 比较轻量，32 跑起来没问题
NUM_EPOCHS = 50
LEARNING_RATE = 1e-4  # Transformer 独立训练建议稍微大一点的学习率
MIXUP_ALPHA = 0.2
...
print(f"🚀 启动 Swin Transformer 训练 | 设备: {device} | 图像尺寸: {IMG_SIZE}x{IMG_SIZE}")
...
def evaluate_model_detailed(model, dataloader, device, classes):
    ...
    print("✅ Swin 混淆矩阵已保存为: ./Swin_confusion_matrix.png")
...
def main():
    ...
    # 🟢 核心：加载 Swin Transformer Tiny 版本
    print("加载模型 (Swin Transformer Tiny)...")
