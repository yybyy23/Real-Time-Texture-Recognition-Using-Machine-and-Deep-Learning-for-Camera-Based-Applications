...
def create_model(num_classes):
    ...
    # 冻结策略：MobileNetV2 的 features 有 18 层
    # 我们冻结前 10 层，让后半部分参与微调
    for i, child in enumerate(model.features.children()):
        if i < 10:
            for param in child.parameters():
                param.requires_grad = False

    # 修改分类器 (MobileNetV2 的分类头在 classifier[1])
...
def train_model(model, train_loader, val_loader, num_epochs=50):
    ...
    print(f"开始训练 (MobileNetV2 + Mixup)...")
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f'Epoch {epoch + 1}/{num_epochs} | Train Loss: {epoch_loss:.4f} | Val Acc: {val_epoch_acc:.4f} | LR: {scheduler.get_last_lr()[0]:.6f}')
...
def plot_training_history(train_losses, val_losses, train_accs, val_accs):
    ...
    plt.tight_layout()
    plt.savefig('training_results_mobilenet_v2.png', dpi=300)
    print("训练图表已保存至: training_results_mobilenet_v2.png")
    plt.close()
...
def save_model(model, class_names):
    ...
    torch.save(checkpoint, 'texture_classifier_mobilenet.pth')
    print("最佳模型已保存: texture_classifier_mobilenet.pth")
...
def main():
    ...
    num_classes = len(class_names)
    print(f"类别数: {num_classes}, 训练集大小: {len(train_dataset)}")
